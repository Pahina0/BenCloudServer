/**** Fix get_population: only require growth base year when projection needed ****/
...
declare
	...
	_base_year integer;
begin
-- 2026-04-23 Fix: datasets without growth tables should still work for known years
...
-- there is no mapping in crosswalk_dataset to do a conversion
	if (_source_grid_definition_id != _output_grid_definition_id) then
		...
		if _year = ANY(_known_pop_years) then
			...
			-- Crosswalk needed, Projection needed -> requires base year
		...
		if _year = ANY(_known_pop_years) then
			...
			-- No crosswalk needed, projection needed -> requires base year
			SELECT base_pop_year
			FROM data.mat_pop_dataset_base_year mpdby
			WHERE mpdby.pop_dataset_id = _dataset_id LIMIT 1
				into _base_year;
